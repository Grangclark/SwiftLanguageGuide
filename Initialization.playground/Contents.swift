...
// 2025年10月31日[金]
// Vehicle クラスは、その唯一の格納プロパティにデフォルト値を提供し、カスタムのイニシャライザを提供していません。
// その結果、Default Initializers(デフォルトイニシャライザ)で説明されているように、
// デフォルトのイニシャライザを自動的に受け取ります。
// デフォルトイニシャライザは、(使用可能な場合)常にクラスの指定イニシャライザで、
// numberOfWheels が 0 の新しい Vehicle インスタンスを作成するために使用できます:
let vehicle = Vehicle()
...
// 2025年11月20日[木]
// また、全てのサブクラスの必須イニシャライザの実装の前に、required 修飾子を記述して、
// イニシャライザの必須要件が初期化チェーン内の他のサブクラスに適用されることを示す必要があります。
// 必須指定イニシャライザをオーバーライドするときは、override 修飾子を記述しません:
