...
// アクターとは、コードに可変データへのアクセスを順番に行わせることで、そのデータへのアクセスを保護するオブジェクトです。
// 多くのプログラムにおいて最も重要なアクターは、メインアクター(Main Actor)です。
// アプリでは、メインアクターは UI を表示するために使用される全てのデータを保護します。
// メインアクターは、UI のレンダリング、UI イベントの処理、そしてあなたが書いた UI のクエリや更新が必要なコードを順番に実行します。
@MainActor
func show(_: Data) {
    // ... 写真を表示するためのUIコード ...
}
